mutable struct AntGatherEnv{SIM<:MJSim,S,O} <: AbstractGatherEnv
    ...
    function AntGatherEnv(
        sim::MJSim;
        structure = WorldStructure.wall_structure,
        napples = 8,
        nbombs = 8,
        activity_range = 10.0,
        ...
        env = new{typeof(sim),typeof(sspace),typeof(ospace)}(
            ...
            sensor_range,
            sensor_span,
            Dict([(Apple(), i) for i = 1:napples]),
            Dict([(Bomb(), i) for i = 1:nbombs]),
            rng,
            viz,
        )
        reset!(env)
...
function LyceumBase.tconstruct(
    ...
    outfile = "antgather-tmp$(rand(1:1000000)).xml"
    WorldStructure.create_world(
        antmodelpath,
        napples,
        nbombs,
        nbins;
    ...
    envs = Tuple(
        AntGatherEnv(
            ...
            viz = viz,
        ) for s in LyceumBase.tconstruct(MJSim, n, modelpath, skip = 5)
    )
...
@inline LyceumMuJoCo._torso_ang(shapedstate::ShapedView, ::AntGatherEnv) =
